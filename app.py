...
def load_data():
    ...
    # Limpeza básica fica aqui para ser feita apenas uma vez
    df["Dt. Entrada"] = pd.to_datetime(df["Dt. Entrada"], errors="coerce")
    df.dropna(subset=["Dt. Entrada"], inplace=True) # Remove linhas onde a data é inválida
...
status = st.sidebar.multiselet("Selecione o Status", options=df["STATUS"].unique(), default=df["STATUS"].unique())
...
with col_graf1:
    # 1. Evolução mensal (ORDENADO CORRETAMENTE)
    meses_ordenados = [
    ...
    fig1 = px.bar(processos_mes, x="Mês", y="Protocolo", title="Processos por Mês", labels={'Protocolo': 'Quantidade'})
